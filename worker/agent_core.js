/**
 * 这个文件实现什么功能：集中管理 Worker 侧主 agent 核心配置与消息组装。
 * 它负责什么：提供人格层/安全层/场景层/定制层/版本层配置，以及统一的 system message 构建入口。
 * 它不负责什么：不处理 HTTP 路由、鉴权、数据库、上游 API 调用。
 * 对外暴露什么：AGENT_CORE_VERSION、AGENT_CORE_LAYERS、buildAgentMessages、buildSystemPrompt。
 * 依赖哪些关键模块：无（纯配置 + 纯函数）。
 */

// #下面代码实现的功能：定义主 agent 核心版本，后续可用于灰度/回滚/评测对照
export const AGENT_CORE_VERSION = "v1";

// #下面代码实现的功能：集中定义主 agent 各层文案，避免散落在路由或请求处理逻辑里
export const AGENT_CORE_LAYERS = {
  persona: [
...
export function buildSystemPrompt(extraCustomization = []) {
  const normalizedCustomization = Array.isArray(extraCustomization)
    ? extraCustomization.filter((item) => typeof item === "string" && item.trim()).map((item) => item.trim())
    : [];

  const sections = [
    ...AGENT_CORE_LAYERS.persona,
    "对话风格要求：",
    ...AGENT_CORE_LAYERS.style.map((line, index) => `${index + 1}) ${line}`),
    ...AGENT_CORE_LAYERS.safety.map((line, index) => `${index + 4}) ${line}`),
...
export function buildAgentMessages({ history, userMessage, customization = [] }) {
  return [
